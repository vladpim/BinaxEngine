...
// Реализация initPostProcessing и renderFullScreenQuad (без изменений)
void initPostProcessing(int width, int height) {
...
int main() {
    ...
    for (int i = 1; i < __argc; ++i) {
        ...
        if (arg == "--game" && i + 1 < __argc) {
            isGameMode = true;
            sceneToLoad = __argv[++i];
        } else if (arg == "--help") {
            std::cout << "Usage:\n  BinaxEngine                - Editor mode\n  BinaxEngine --game scene.bxlvl - Run game with specified scene\n";
            glfwTerminate();
            return 0;
        }
    }
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Binax Engine", NULL, NULL);
    if (!window) {
    ...
    // Загружаем скайбокс и envCubemap (общие для обоих режимов)
    skybox.Load(
    ...
    if (isGameMode) {
        if (!sceneToLoad.empty()) {
            g_SceneManager.LoadScene(sceneToLoad);
        } else {
            std::cerr << "[Main] No scene specified for game mode." << std::endl;
            return -1;
        }
        RunGameLoop(window);
        g_EditorUI.Shutdown();
    ...
    // --- Редакторский цикл (без изменений) ---
...
void RunGameLoop(GLFWwindow* window) {
    ...
    auto& settings = g_EditorUI.GetSettings();
    settings.grid_enabled = false;      // сетка не нужна
    settings.show_gizmo = false;        // гимо не нужны (хотя мы их и так не рисуем)
    settings.wireframe_mode = false;    // каркасный режим тоже выключаем

    g_SceneManager.StartPlay();
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // --- Сетка пропущена (уже отключена через settings.grid_enabled = false) ---
        ...
        // --- ImGui и гимо пропущены ---

        glfwSwapBuffers(window);
        glfwPollEvents();
    }

    g_SceneManager.StopPlay();
    std::cout << "[Game] Game loop finished." << std::endl;
}
